...
class ManufactureController extends Controller{
    ...
    public function __construct(Manufacture $model){
        $this->model=$model;
        $this->admin_base_url="manufactures.index";
        $this->admin_view="admin.manufactures";
    }
    ...
    public function store(Request $request){
        $validator = Validator::make($request->all(), [
            //'seo_url' => 'required|unique:manufactures,seo_url',
            'title' => 'required'
        ]);   
        if($validator->fails()){
        ...
        if ($request->hasFile("image")) {
            $data['image'] = Upload::fileUpload($request->file("image"),"manufactures");
        }
        if ($request->hasFile("bannerImage")) {
            $data['bannerImage'] = Upload::fileUpload($request->file("bannerImage"),"manufactures");
        }
    ...
    public function update(Request $request, $id){
        $validator = Validator::make($request->all(), [
            'seo_url' => 'required|unique:manufactures,seo_url,'.$id,
            'title' => 'required'
        ]);   
        if($validator->fails()){
            return redirect()->back()->withInput()->with("danger",$validator->errors()->first())->withErrors($validator->errors());
