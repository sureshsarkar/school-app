...
class CareerController extends Controller{
    ...
    public function __construct(Career $model){
        $this->model=$model;
        $this->admin_base_url="careers.index";
        $this->admin_view="admin.careers";
    }
    ...
    public function store(Request $request){
      $data=$request->all();
        if ($request->hasFile("image")) {
            $data['image'] = Upload::fileUpload($request->file("image"),"careers");
        }
        if ($request->hasFile("bannerImage")) {
            $data['bannerImage'] = Upload::fileUpload($request->file("bannerImage"),"careers");
        }
        if($request->skill){
          $arr_string=  implode(",",$request->skill);
        }else{
            $arr_string=null;
        }
        $data['skill']=$arr_string;
