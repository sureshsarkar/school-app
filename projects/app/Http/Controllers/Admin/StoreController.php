...
class StoreController extends Controller{
    ...
    public function __construct(Store $model){
        $this->model=$model;
        $this->admin_base_url="stores.index";
        $this->admin_view="admin.stores";
    }
    ...
    public function index(){
        ...
        // foreach($data as $a){
        //     $ar=Store::find($a->id);
        //     $ar->meta_title=$a->title;
        //     $ar->meta_keywords=$a->title;
        //     $ar->meta_description=$a->title;



        //     $ar->seo_url=Str::of($a->title)->slug('-')->value;
        //     $ar->save();
        // }
    ...
    public function store(Request $request){
        $validator = Validator::make($request->all(), [
           // 'seo_url' => 'required|unique:stores,seo_url',
            'title' => 'required'
        ]);   
        if($validator->fails()){
        ...
        if ($request->hasFile("image")) {
            $data['image'] = Upload::fileUpload($request->file("image"),"stores");
        }
        if ($request->hasFile("bannerImage")) {
            $data['bannerImage'] = Upload::fileUpload($request->file("bannerImage"),"stores");
        }
    ...
    public function update(Request $request, $id){
        $validator = Validator::make($request->all(), [
            'seo_url' => 'required|unique:stores,seo_url,'.$id,
            'title' => 'required'
        ]);   
        if($validator->fails()){
            return redirect()->back()->withInput()->with("danger",$validator->errors()->first())->withErrors($validator->errors());
